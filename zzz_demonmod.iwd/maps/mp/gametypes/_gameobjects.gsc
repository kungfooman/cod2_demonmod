...
onPlayerSpawned()
{
	...
	for(;;)
	{
		...
		self.touchTriggersID = undefined;
	}
}
...
//	trace = bulletTrace( self.safeOrigin + (0,0,20), self.safeOrigin - (0,0,20), false, undefined );
	if( isDefined( self.carrier ) )
	{
		trace = PhysicsTrace( self.carrier.origin + (0,0,20), self.carrier.origin - (0,0,2000), false, self.carrier.body );
		angleTrace = bulletTrace( self.carrier.origin + (0,0,20), self.carrier.origin - (0,0,2000), false, self.carrier.body );
	}
	else
	{
		trace = PhysicsTrace( self.safeOrigin + (0,0,20), self.safeOrigin - (0,0,20), false, undefined );
	...
	if( isDefined( trace ) )
	{
		...
		if( angleTrace["fraction"] < 1 && distance( angleTrace["position"], trace ) < 10.0 )
		{
			...
			forward = vectornormalize( forward - maps\mp\_utility::vectorScale( angleTrace["normal"], vectordot( forward, angleTrace["normal"] ) ) );
...
createUseObject( ownerTeam, trigger, visuals, offset )
{
	...
	if( isDefined( useObject.objPoints["allies"] ) && isDefined( useObject.objPoints["axis"] ) )
	{
...
{
	touchingPlayer = undefined;
	player = getEntArray( "player", "classname" );
	for( i = 0; i < player.size; i++ )
	{
		for( j=0; j < self.playerTouching.size; j++ )
			if( player[i] getEntityNumber() == self.playerTouching[0] && player[i].pers["team"] == self.claimTeam )
				touchingPlayer = player[i];
	}
...
useObjectProxThink()
{
	...
	while( true )
	{
		if( self.useTime && self.curProgress >= self.useTime )
		{
			...
			creditPlayer = self getEarliestClaimPlayer();
...
triggerTouchThink( object )
{
	...
	object.playerTouching[object.playerTouching.size] = self getEntityNumber();
	self.touchTriggersID = object.entNum;
...
updateProxBar( object, forceRemove )
{
	if( forceRemove || !object canInteractWith( self.pers["team"] ) || self.pers["team"] != object.claimTeam )
	{
		if( isDefined( self.proxBar ) )
		{
			...
			self.proxBar = undefined;
		}

		if( isDefined( self.proxBarText ) )
		{
			self.proxBarText destroyElem();
			self.proxBarText = undefined;
		}
...
useHoldThink( player )
{
	...
	if( isDefined( useWeapon ) )
	{
		...
		if( useWeapon == player getweaponslotweapon( "primary"  ) )
			currentslot = "primary";
		else
			currentslot = "primaryb";
		
		player giveWeapon( useWeapon );
		player setWeaponSlotAmmo( currentslot, 0 );
		player setweaponslotclipammo( currentslot, 0 );
	...
	while( isAlive( player ) && player isTouching( self.trigger ) && player useButtonPressed() && !player.throwingGrenade && !player meleeButtonPressed() && self.curProgress < self.useTime && (self.useRate || waitForWeapon) && !(waitForWeapon && timedOut > maxWaitTime) )
	{
		...
		if( self.curProgress >= self.useTime )
		{
			...
			if( isDefined( useWeapon ) )
			{
				...
				player giveWeapon( useWeapon );
				player setWeaponSlotAmmo( currentslot, 1 );
				player setweaponslotclipammo( currentslot, 1 );
				if( lastWeapon != "none" )
					player switchToWeapon( lastWeapon );
				else
					player takeWeapon( useWeapon );
...
disableObject()
{
	...
	self.trigger maps\mp\_utility::triggerOff();
...
enableObject()
{
	...
	self.trigger maps\mp\_utility::triggerOn();
